...
func (s *pluginStore) loadUser(mattermostUserID string) (*user, error) {
	u := &user{}
	err := s.userStore.loadJSON(fmt.Sprintf("%s%s", UserStoreKeyPrefix, mattermostUserID), u)
	if err != nil {
...
func (s *pluginStore) storeUser(mattermostUserID string, u *user) error {
	...
	err := s.userStore.setJSON(fmt.Sprintf("%s%s", UserStoreKeyPrefix, mattermostUserID), u)
	if err != nil {
